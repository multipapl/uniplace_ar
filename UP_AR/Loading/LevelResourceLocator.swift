...
//  `Terrace`) plus typed media folders (`ProbesTextures`, `Videos`, `Audio`, `SFX`); layer file names
//  are globally unique (LO_/TR_ prefixes), so we just search every content subfolder and fall back to a
//  flat bundle lookup — no scene→folder mapping needed.
//
